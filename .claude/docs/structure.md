...
| 분류 | 위치 | 역할 |
|---|---|---|
| command | `.claude/agents/commands/` | 사용자가 직접 호출하는 명령어 에이전트 |
| agent | `.claude/agents/` (서브디렉토리) | Claude가 작업 중 호출하는 전문 에이전트 |
| docs | `.claude/docs/` | 프로젝트 정보 — 에이전트가 탐색·작성 시 참조 |
...
│   ├── structure.md                     [docs]    이 문서 — .claude/ 구조 지도
...
- command 추가 → `.claude/agents/commands/<name>.md`, CLAUDE.md Commands 섹션에 등록
- agent 추가 → `.claude/agents/<type>/<name>.md`, CLAUDE.md Agents 섹션에 등록
- docs 추가 → `.claude/docs/<name>.md`, CLAUDE.md Docs 섹션에 등록
